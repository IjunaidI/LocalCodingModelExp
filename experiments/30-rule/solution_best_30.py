...
def process_calculations(csv_path):
    ...
    def process_row(row):
        ...
        # Final discounts
        total_receivables *= (1 - 0.10) if city in CAPITAL_CITIES else 1.0
        total_receivables *= (1 - 0.02) if senior_citizen == "yes" else 1.0
        total_receivables *= (1 - 0.05) if first_order == "yes" else 1.0
        total_receivables *= (1 - 0.04) if product_category == "luxury" else 1.0
        total_receivables *= (1 - 0.03) if weight > 20 else 1.0
        total_receivables *= (1 - 0.07) if customer == "strategic" and loyalty_status == "platinum" and volume >= 1000 and correction_fee == 0.0 else 1.0
        total_receivables *= (1 - 0.06) if season == "spring" and product_category == "luxury" and processing_speed == "same-day" else 1.0
        total_receivables *= (1 - 0.04) if status == "corporate" and city in CAPITAL_CITIES and payment_type != "credit-card" and tax_verified == "yes" else 1.0
        total_receivables *= (1 - 0.08) if transaction_type == "international" and customer == "strategic" and first_order != "yes" and volume > 5000 else 1.0
        total_receivables *= (1 - 0.05) if loyalty_status == "platinum" and referral_active == "yes" and is_high_risk != "yes" and tax_verified == "yes" else 1.0
