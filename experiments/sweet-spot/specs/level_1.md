<!-- LEVEL 1 — Prescriptive pseudocode (max scaffold). Mirrors the validated v2
     dialect: literal dicts, additive Rule 4 with "no elif", yes/no boolean, bare
     decimals, step-by-step pseudocode. Dataset: yes/no. Everything above the first
     "#" heading is stripped before the spec reaches the model. -->
# Pricing & commission calculation spec

Build `process_calculations(csv_path)`. It reads a CSV and returns a list of dicts, one
per row, each containing the key `total_receivables`.
...
7. **Region loading.** Start `region_loading = 0.0`. If `region` is "emea", set `region_loading = 0.10 * receivable_after_discount`.
8. **Payment loading.** Start `payment_loading = 0.0`. If `payment type` is "credit-card", set `payment_loading = 0.025 * receivable_after_discount`.
9. **Commission.**
